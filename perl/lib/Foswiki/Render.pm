...
sub internalLink {
    ...
    #$topic = ucfirst($topic); favio flamingo wants capital letters
...
sub _handleSquareBracketedLink {
    ...
    #$link = ucfirst($link); favio flamingo wants capital letters
...
sub renderFORMFIELD {
	...
	my $formTopicObject = Foswiki::Meta->new( $this->{session}, $topicObject->web, $topicObject->topic, '' );
	...
	my $found  = 0;
	my $text   = $format;
	my $permission = $formTopicObject->haveAccess( 'VIEW', $cUID );

	if($permission){
		# has view permissions
		
		# get the field row  {'name' => $name, 'value' => $value, 'title' => $name, 'form_key' => $form_key} 
		my $field_row = $this->{session}->{store}->getFormField( $formTopicObject, undef , $formField);
		#require Data::Dumper;
		#my $kaw = Data::Dumper::Dumper($field_row);
		#die "($formField)\n($kaw)";
		...
		# this is the topic_history_key of the form definition
		my $form_key = $field_row->{'form_key'};
		my ($fweb,$ftopic,$frev) = $this->{session}->{store}->getWTRFromTHKey($form_key);
		#die "Form:($name,$value,$form_key,$fweb,$ftopic,$frev)";
		my $fname = $fweb.'.'.$ftopic; 
		if ( $text =~ m/\$form/ ) {		
                        $text =~ s/\$form/$fname/g;
                }
	...
	#die "Death($permission)($formField):($text)";# if $text ne $value;
    return $text;
}
