...
sub INCLUDE {
	...
	# check DBIStore if this thing already exists
	my $sitehandler = $this->{store}->{site_handler};
	# use %control as the key
	require Digest::SHA1;
	require Data::Dumper;
	my $cachekey001 = Digest::SHA1::sha1(Data::Dumper::Dumper(%control));
	if(defined $sitehandler->{include_cache}->{$cachekey001.$args} ){
		return $sitehandler->{include_cache}->{$cachekey001.$args};
	}
    ...
    my $includedTopicObject = Foswiki::Meta->new( $this, $includedWeb, $includedTopic, $control{rev} );
    ...
    try {
		...
		if($control{section}){
			...
			#$text = $this->{store}->fetchIncludeInCache($includingTopicObject->web,$includingTopicObject->topic,$control{section});
		}
		unless($text){
			# then do the ugly thing and load it.
			$includedTopicObject->load();
        ...
        if ( $interesting and ( length($text) eq 0 ) ) {
			...
			# stuff this text into the Memcache, before expanding macros
			$this->{store}->putIncludeInCache($includingTopicObject->web,$includingTopicObject->topic,$control{section},$text);
	...
	# cache this in DBIStore
	# TODO: super bad setup here, change it soon!
	# hopefully this information only lasts one request
	$sitehandler->{include_cache}->{$cachekey001.$args} = $text;
