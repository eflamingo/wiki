...
sub parseTables {
    my ( $inText, $inTopic, $inWeb, $options ) = @_;
    
    # $options was added b/c of parsing irregularities with beforeCommanTagHandler conflicting with BeforeSaveHandler
	
	
    return if defined $tableMatrix{$inWeb}{$inTopic} && !$options->{'overwrite'};
	#die "InText:\n$inText";
...
# get the table data for topic (web,topic)
sub tableMatrix {
	my ($web,$topic) = @_;
	return $tableMatrix{$web}{$topic};
}

sub convertToSave {
	# #$plugins->dispatch( 'beforeSaveHandler', $text, $this->{_topic},$this->{_web}, $this );
    my ($text, $topic_name ,$web_name, $topicObject) = @_;
	
	Foswiki::Plugins::EditTablePlugin::Core::parseTables( $text, $topic_name, $web_name , { 'overwrite' => 1});
	my $tableData = Foswiki::Plugins::EditTablePlugin::Core::tableMatrix($web_name,$topic_name);
	
	my @json_tables;
	my $table_count = 0;
	#my @scum;
	foreach my $editTableData (@{ $tableData->{editTableDataList} } )
    {
    	my $row_num = 0;
    	my @table_rows;
    	#### figure out the columns ####
    	Foswiki::Plugins::EditTablePlugin::Core::handleEditTableTag( $web_name, $topic_name, $editTableData->{'params'} );
    	my $params = $editTableData->{'params'};
    	##### handle EditTable Tag #####-----------------------------------
		my $editTableTag   = $editTableData->{'tagline'};
		
		# process the tag contents
		handleEditTableTag( $web_name, $topic_name, $editTableData->{'params'} );
		my ($headerRow,$formating,$footerRow);
		my @column_names;
		my @formats_per_column;
		my $include_topic = Foswiki::Func::extractNameValuePair(  $editTableData->{'params'}, "include" );
		my ($incWeb,$incTopic) = Foswiki::Func::normalizeWebTopicName($topicObject->web,$include_topic);
		if($include_topic){
			
			# get the Blob from the other Topic, then do JSON backwards to get the formating
			my $included_json = _getJsonFromInclude($incWeb.'.'.$incTopic);

			my ($column_names_ref,$formats_per_column_ref) = _extractDefFromJson($included_json);
			@column_names = @{$column_names_ref};
			@formats_per_column = @{$formats_per_column_ref};
			
		}
		else{
			# formating is in the current Topic
			$headerRow  = Foswiki::Func::extractNameValuePair(  $editTableData->{'params'}, "header" );
			$footerRow  = Foswiki::Func::extractNameValuePair(  $editTableData->{'params'}, "footer" );
			$formating  = Foswiki::Func::extractNameValuePair(  $editTableData->{'params'}, "format" );
			@column_names = _getColumnNames($headerRow);
			@formats_per_column = _getFormatsByColumn($formating);	
		} 

		my $preColCount = scalar(@column_names);
    	foreach my $fullrow (@{$editTableData->{'lines'}}){
    		#push(@scum,$fullrow);
    		$fullrow =~ /^\|(.*)\|$/;
    		my @cells = split( /\|/, $1 );
    		...
    		foreach my $cell (@cells){
    			...
    			if($row_num == 0 && !$headerRow){
    				# use numbers if the columns don't match with the headers
    				push(@column_names,$col_num) unless $column_names[$col_num];		
    			}
    			# Not header, can just stuff body of table #
    			if((!$headerRow)||($row_num > 0 && $headerRow)){
    				# guts of table
    				$cell =~ m/^\s*(.*?)\s*$/;
    				$row_ref->{$column_names[$col_num]} = _convertValueByColumn($formats_per_column[$col_num],$1);
    				$row_ref->{'_row'} = $row_num;
    				
    			}
    			$col_num += 1;
    		}
    		# hopefully, no one picks r001 as a column name
    		push(@table_rows,$row_ref) if $row_ref->{'_row'}; 
    		$row_num += 1;
    	}
    	#die "Scum:\n".join("\n------\n",@scum);
    	my $json_data = JSON::to_json(\@table_rows, {utf8 => 1, pretty => 1});
    	# die "Json:\n$json_data"; does not show that row was changed ---------------------
    	my ($def_hash,$def_key) = _getJsonDef(\@column_names,\@formats_per_column);
    	my $json_def1 = JSON::to_json({'column'=>\@column_names,'format'=>\@formats_per_column}, {utf8 => 1, pretty => 1});
    	#die "Json def: $json_def1";
		
		## TODO: put the register Meta somewhere around here ##
    	$topicObject->putKeyed( 'EDITTABLE',{ 'name' => $table_count, 'data' => \@table_rows, 'definition' => $def_hash, 'definition_key' => $def_key , 'included'=> $include_topic} );	
    	$table_count += 1;
    }
    
	
}
# convert the cell_value to a topic_key, user_key, etc... based on the format param value in the EditTable Tag
sub _convertValueByColumn {
	my $format_type = shift;
	my $cell_value = shift;
	
	return $cell_value unless $format_type->{'type'} eq 'topic';
	...
	return $topic_handler->convert_Var('WT','Topics',$cell_value,'in');
}

# | *Name* | *Value* | is the format for the headers
sub _getColumnNames {
	my $header = shift;
	$header =~ /^\|(.*)\|$/;
	my @cols = split( /\|/, $1 );
...
# returns an array of array references, each array ref is the list of terms in one column -> text, 35, blah
sub _getFormatsByColumn {
	my $formating = shift;
	$formating =~ /^\s*\|(.*)\|\s*$/;
	my @cols = split( /\|/, $1 );
	my @newCols;
	...
	foreach my $cell (@cols){
		...
		$cell_ref->{'type'} = 'text' unless $cell_ref->{'type'};
		$cell_ref->{'value'} = 35 unless $cell_ref->{'value'};
		push(@newCols,$cell_ref);
		$i += 1;
	}
	return @newCols;
}

# (\@cols,\@format)-> $def_hash
sub _getJsonDef {
	my $col_ref = shift;
	my $format_ref = shift;
	my @Cols = @$col_ref;
	...
	return undef unless scalar(@$col_ref) == scalar(@$format_ref);
	my $DefByColumn;
	my $i = 0;
	foreach my $col (@$col_ref){
		$DefByColumn->{$col_ref->[$i]} = $format_ref->[$i];
		$i += 1;
	}
	my $json_def = JSON::to_json($DefByColumn, {utf8 => 1, pretty => 1});
	my $def_key = sha1($json_def);
	return ($DefByColumn,$def_key);
}

# $def_hash in JSON -> (\@columns,\@formats)
sub _extractDefFromJson {
	my $json_def = shift;
	#die "JSON:\n$json_def\n";
	my $def_hash = JSON::from_json($json_def,{utf8 => 1});
	my @columns;
	my @formats;
	my $rearranger;
	my $i = 0;
	my %alreadyDoneNumbers;
	foreach my $col_name1 (keys %$def_hash){
		my ($type,$value,$other,$colNum) = ($def_hash->{$col_name1}->{'type'},$def_hash->{$col_name1}->{'value'},
												$def_hash->{$col_name1}->{'other'},$def_hash->{$col_name1}->{'_col'});
		#$colNum = $i unless $colNum;
		$rearranger->{$colNum} = {'type'=>$type,'value'=>$value,'other'=>$other,'name'=> $col_name1};
		$i += 1;
	}
	#die "Magic: $i";

	for (my $j  = 0; $j < $i; $j++){
		push(@columns,$rearranger->{$j}->{'name'});
		
		my $ntype = $rearranger->{$j}->{'type'};
		my $nvalue = $rearranger->{$j}->{'value'};
		my $nother = $rearranger->{$j}->{'other'};
		push(@formats,{'type' => $ntype, 'value' => $nvalue, 'other' => $nother});
	}
	die "Columns don't match Formats" if scalar(@columns) != scalar(@formats);
	
	return (\@columns,\@formats);
	
}
# WT -> topic_key -> topic_history_key -> edit_table_key -> definition_blob_key
sub _getJsonFromInclude {
	my $wtpair = shift;
	...
	require Foswiki::Contrib::DBIStoreContrib::TopicHandler;
	my $topic_handler = Foswiki::Contrib::DBIStoreContrib::TopicHandler::->init($site_handler);
	my $topic_key = $topic_handler->convert_Var('WT','Topics',$wtpair,'in');

	# get the Edit Table Key
	require Foswiki::Contrib::DBIStoreContrib::DataformHandler;
	my $df_handler = Foswiki::Contrib::DBIStoreContrib::DataformHandler::->init($site_handler);
	my $json_text = $df_handler->getEditTableDefinitionFromInclude($topic_key);
	return $json_text;
}
1;
...
FROM 
  ...
  foswiki."Sites" s1,
  foswiki."Blob_Store" bs2
WHERE

  bname."value" = 'FreeswitchDialPlanTable'
  AND w1.current_web_name = 'System'
  AND w1.site_key = s1."key"
  AND s1.current_site_name = 'tokyo.e-flamingo.net'
  AND bs2."key" = etd.definition_key
