...
sub _getMapping {
	...
	#TODO: Must fix basemapping quickly!
...
"Broken user mapping $mapping; does not implement login2cUID"
        ...
        unless ($cUID) {
            ...
            # $cUID = $found->[0] if ( $found && scalar(@$found) );  never returns more than one
            $cUID = $found if $found;
        }
    }
    return $cUID;
...
sub eachGroup {
    my $this = shift;
    return $this->{mapping}->eachGroup(@_);
}
...
sub eachGroupMember {
    ...
    return $this->{mapping}->eachGroupMember(@_);
}
...
sub isInAdminGroup {
    ...
    return 1 if $mapping->isInAdminGroup( $cUID);
...
---++ ObjectMethod createNewGroup( $cuid, $group, $create ) -> $boolean
creates a new group
...
sub createNewGroup {
    my ( $this) = shift;
    my $mapping = $this->{mapping};
    return $mapping->createNewGroup( @_ );
}
...
---++ ObjectMethod changeGroupMemberShip( ) -> $boolean
...
sub changeGroupMemberShip {
	my $this = shift;
    my $mapping = $this->{mapping};
    return $mapping->changeGroupMemberShip( @_ );
}
