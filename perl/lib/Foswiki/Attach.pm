...
sub formatVersions {
	...
	# spits out Foswiki::Contrib::DBIStoreContrib::ResultSetAttachment
    ...
    my $i = 0;
    while ( $revIt->hasNext() ) {
    	# iterators have the data
        #my $rev = $revIt->next();
        my $info;
        #my $info = $topicObject->getAttachmentRevisionInfo( $attrs{name}, $rev );
        # attachment name = filename.filetype
        $info->{name} = $revIt->currentRow('file_name').'.'.$revIt->currentRow('file_type');
        $info->{ext} = $revIt->currentRow('file_type');
        $info->{attr} = $revIt->currentRow('file_type');
        $info->{size} = $revIt->currentRow('size');
		# user, date, comment, version (usually fetched via getAttachmentRevisionInfo )
		$info->{user} = $revIt->currentRow('user_key');
		$info->{date} = $revIt->currentRow('timestamp_epoch');
		$info->{version} = $revIt->currentRow('revision');
		$info->{revision} = $revIt->currentRow('revision');
		$info->{comment} = $revIt->currentRow('comment');
		warn "Attaching in Attach: $i\n";
...
sub _expandAttrs {
    ...
    if ( $attr eq 'REV' ) {
        ...
        return '%ICON{"' . $file_ext . '" default="else"}%';
    }
    elsif ( $attr eq 'EXT' ) {
