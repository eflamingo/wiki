...
sub getPreference {
        ...
        # TODO: terrible hack, MACROS are first searched for as preferences
        my $macroslist = { 'FORMFIELD' => 1, 'INCLUDE' => 1, 'SEARCH' => 1};
        return undef if $macroslist->{$key};
        
        my ($web,$topic) = ($this->{session}->{webName},$this->{session}->{topicName});
	...
	require Foswiki::Contrib::DBIStoreContrib::Handler;
 	require Foswiki::Contrib::DBIStoreContrib::MetaPreferenceHandler;
 	require Foswiki::Func;
	
	
	# check cache first
	my $cachevalue;
	#$cachevalue = Foswiki::Func::fetchFromMemcached('get_preference_cache',$web.$topic.$key);
        # when values are cached, we have to put something indicating that nothing was in the database
	#return '' if $cachevalue eq ' ';
	#return $cachevalue unless $cachevalue;
	...
	my $site_handler = Foswiki::Contrib::DBIStoreContrib::Handler::->new();
        ...
        $value =  $mphandler->CascadePreference($web,$topic,$key);
        # cache returned values
        $cachevalue = $value;
        #$cachevalue = ' ' unless $cachevalue;
        Foswiki::Func::putInMemcached('get_preference_cache',$web.$topic.$key,$cachevalue,30);
        #die "($key,$value)" if $key eq 'WEBCOPYRIGHT';
        
        return $value if $value;
=pod	
	my $mpreturn = $mphandler->CascadePreference($web,$topic,$key);
	my $sitelevel = $mpreturn->{'site'}->{$key};
	my $weblevel = $mpreturn->{'web'}->{$key};
	my $userlevel = $mpreturn->{'user'}->{$key};
	my $topiclevel = $mpreturn->{'topic'}->{$key};
	#die "Skin: $sitelevel,$weblevel,$topiclevel\n" if $key eq 'EXTERNAL_DIALPLAN';
	#die "PatternSkin: $sitelevel,$weblevel,$topiclevel\n" if $key eq 'PATTERNSKIN_COLUMNS';
	$value = $sitelevel;
	$value = $weblevel if $weblevel;
	$value = $userlevel if $userlevel;
	$value = $topiclevel if $topiclevel;

	#return $value;
	return $topiclevel if $topiclevel;
	return $userlevel if $userlevel;
	return $weblevel if $weblevel;
	return $sitelevel;
	#return undef;
	# TODO: get rid of FORM and FIELDS in preferences
	# $key might be a field name
=cut
