...
---++ StaticMethod createNewGroup($session)
creates a new group
...
---++ StaticMethod changeGroupMemberShip($session)
...
sub changeGroupMemberShip {
	...
	# we receive Web.Topic pairs, need to convert into Topic Keys
    my $addUserText = $query->param('addUsers');
    my $removeUserText = $query->param('removeUsers');
    my @addUserList = split(',',$addUserText);
    my @removeUserList = split(',',$removeUserText);
	

	# if the GroupName is not specified, then the current web.topic is used
	# no need to normalize the group name, it is done in Foswiki::Func
    my $groupName = $query->param('GroupName') || $web.'.'.$topic;
    
    # kill this action if add and remove User Lists are empty
    if ( scalar(@addUserList) == 0 && scalar(@removeUserList) == 0 ) {
        throw Foswiki::OopsException(
            'attention',
            def    => 'no_group_specified_for_add_to_group',
            web    => $web,
            topic  => $topic,
            params => [$addUserText.$removeUserText]
        );
    }    
    # execute mofo
    Foswiki::Func::changeGroupMemberShip($groupName,\@addUserList,\@removeUserList);
	# on success
    throw Foswiki::OopsException(
        'attention',
        status => 200,
...
sub _loadPendingRegistration {
    ...
    unless ( -f $file ) {
        ...
        if ( $users ) {
            #if ( scalar( @{$users} ) && $session->{users}->userExists( $users->[0] ) )  
            if ($session->{users}->userExists( $users ) )
            {
                throw Foswiki::OopsException(
                    'attention',
