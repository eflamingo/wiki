...
sub generate {
    ...
    # array of Topic Keys, not Topic names
    my @forms = Foswiki::Form::getAvailableForms($topicObject);
    unshift( @forms, 'none' );

    my $formList      = '';
    my $formElemCount = 0;
    foreach my $form_key (@forms) {
        my ($fweb,$ftopic) = Foswiki::Func::getWTFromTopicKey($form_key);
        
		my $form = "$fweb.$ftopic" if $fweb && $ftopic;
		$form = "none" unless $fweb && $ftopic;
        ...
        my $props      = {
            ...
            name  => 'formtemplate',
            id    => $formElemId,
	    # this value should not be the topic key, but be the web.topic name pair
            value => $form
        };
        my ($formNameW,$formNameT) = $session->normalizeWebTopicName( $topicObject->web, $formName );
        $formName = "$formNameW.$formNameT";
        $props->{checked} = 'checked' if $form eq $formName;
        $formList .= CGI::input($props);
        #my ( $formWeb, $formTopic ) = $session->normalizeWebTopicName( $topicObject->web, $form );
        #my $temp01 = $form if $form eq "none";
        my $temp01 = $form if $form eq "none";
        
        $temp01 = '[[' . $fweb . '.' . $ftopic . '][' . $ftopic . ']]' unless $form eq "none";
        my $formLabelContent = '&nbsp;'.$temp01;
