...
sub _resetUsersPassword {
    ...
    unless ( $users->setPassword( $user, $password, 1 ) ) {
        $$pMess .= $session->inlineAlert( 'alertsnohtml', 'reset_bad', $login ); # changed last argument from $user to $login
    ...
    my @em   = $users->getEmails($user); #argument is $user_key or $cUID
 
    my $sent = 0;
    if ( !scalar(@em) ) {
        $$pMess .=
          $session->inlineAlert( 'alertsnohtml', 'no_email_for', $login ); # changed last argument from $user to $login
    }
    else {
