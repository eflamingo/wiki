...
sub _moveTopicOrAttachment {
    ...
    if ($attachment) {
        ...
        # no need for anything past this point, we only need to move the topic
=pod
        ...
        #$text = $session->renderer->forEachLine( $text, \&_replaceTopicReferences, $options );
        $to->text($text);
        ...
        #_updateReferringTopics( $session, $refs, \&_replaceTopicReferences, $options );
=cut
    }
}
