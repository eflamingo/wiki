...
sub loadSession {
        ...
        && ( $session->{request} && $session->{request}->param('logout') ) )
    {
        ...
        if ($sudoUser) {
            ...
            #die "Super Before($authUser,$redirectUrl)";
        }
    }
    #die "Before($x1,$authUser,$defaultUser)";
    $session->{request}->delete('logout');
...
#		remember there was a time related problem causing sessions to be instantly deleted
#		$lat = $lat + 9*60*60; # need to add 9 hours b/c of weird timezone screw up JST = UTC + 9
...
#die "deauthenticating";
    ...
    #die "nothing ($authUser)(".$Foswiki::cfg{DefaultUserLogin}.")";
}
