...
################### Perform The Search
	# before we parse the search lets do some regex magic!
	my $old_search_string = $searchString;
=pod
	# change \\" to $dollarquot
	$searchString =~ s/(\\\\")/\$dollarquot/g;
	# change \\% to $dollarpercent
	$searchString =~ s/(\\\\%)/\$dollarpercent/g;
	# change \\$ to $dollardollar
	$searchString =~ s/(\\\\$)/\$dollardollar/g;	
	
	# change \" to $quot
	$searchString =~ s/(\\")/\$quot/g;
	# change \% to $percent
	$searchString =~ s/(\\%)/\$percent/g;
	# change \$ to $dollar
	$searchString =~ s/(\\%)/\$dollar/g;	
die "$old_search_string\n\n\n\n$searchString";
#=cut
	# first, let us string out form searches in the form of FORMNAME/FIELDNAME = 'value..'
	# into (form.name = 'FORMNAME' and form.field = 'FIELDNAME' and form.value = 'value')
	# ('[^'~=]+'|[^'~=\s]+)/('[^'~=\s]+'|[^'~=\s]+)(\s*[~=]\s*)('[^'~=\s]+') for forms

	my $tableNumber = 999;
	while($searchString =~ m/('([^'~=]+)'|([^'~=\s]+))\/('([^'~=]+)'|([^'~=\s]+))\s*(=|~=|!=|>|<|~)\s*'([^'"]+)'/g){
		$searchString =~ s/('([^'~=]+)'|([^'~=\s]+))\/('([^'~=]+)'|([^'~=\s]+))\s*(=|~=|!=|>|<|~)\s*'([^'"]+)'/ form_$tableNumber.name = '$2$3' and form_$tableNumber.field = '$5$6' and form_$tableNumber.value $7 '$8' /;
		$tableNumber += 1; 
	}
=cut
...
sub formatResults {
    ...
    # check to see if infoCache is a  Foswiki::Contrib::DBIStoreContrib::ResultSet object
    my $infoCachetype = ref($infoCache);
    return formatResultsListIterator( $this, $query, $infoCache, $params ) unless $infoCachetype eq 'Foswiki::Contrib::DBIStoreContrib::ResultSet';
    # Use the $users variable to get a list of groups that this user is a member of
    # $users->getGroups blah....
...
#paging - this code should be hidden in the InfoCache iterator, but atm, that won't let me do multi-web
        ...
        # CAN'T do this part b/c we don't know how many topics will be returned
    ...
    while ( $infoCache->hasNext() ) {
        #my $listItem = $infoCache->next(); # Is the TOPIC NAME
        #ASSERT( defined($listItem) ) if DEBUG;
        
        # check the VIEW permissions for each Topic false for can't; true for can view
        next unless $infoCache->checkPermission('VIEW',undef);
        ...
        #############################################################
        #TOPIC specific
        my $topic = $infoCache->currentRow('topic_name');
        $web = $infoCache->currentRow('web_name');
        my $text = $infoCache->currentRow('summary');
	# this returns a Meta Object already loaded with the current row information
	my $info = $infoCache->getCurrentTopicObj($session);
        ...
        if (
            ...
            ( $infoCache->isa('Foswiki::Search::InfoCache') ) or
            ( $infoCache->isa('Foswiki::Contrib::DBIStoreContrib::ResultSet') )
          )                                                         #FORMAT

        {
            #( $web, $topic ) = Foswiki::Func::normalizeWebTopicName( '', $listItem ); # unnecessary
...
#                              s/\n$//os;    # remove trailing new line
            ...
            if ( $formatDefined and ( $format ne '' ) ) {
                ...
                $out = $this->formatResult(
                    $format,
                    $info || $webObject,    #SMELL: horrid hack
                    $text,
                    $searchOptions,
                    {
  ...
  #      $infoCache->getRev1Info( $topic, "create$1" )/ges;
                        '\$createlongdate' => sub {
                            ...
                            return $infoCache->currentRow('revision');#$this->metacache->get( $web, $topic )->{tom}->getRev1Info("createlongdate");
                        },
                        '\$createusername' => sub {
                            return $infoCache->currentRow('author');#$this->metacache->get( $web, $topic )->{tom}->getRev1Info("createusername");
                        },
                        '\$createwikiname' => sub {
                            return $infoCache->currentRow('wikiusername');#$this->metacache->get( $web, $topic )->{tom}->getRev1Info("createwikiname");
                        },
                        '\$createwikiusername' => sub {
                            return $infoCache->currentRow('wikiusername');#$this->metacache->get( $web, $topic )->{tom}->getRev1Info("createwikiusername");
                        },

                   #TODO: hacky bits that need to be moved out of formatResult()
                        '$revNum' => sub { return ( $infoCache->currentRow('revision') || 0 ); },
