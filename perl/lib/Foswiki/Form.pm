...
sub new {
    ...
    unless ($this) {
        ...
        # get the Store handler to fetch parseFormDefinition
		my $fields_ref = $session->{store}->parseFormDefinition($this,$vweb, $vtopic);

        # Got to have either a def or a form defined
        unless ( $def || $fields_ref ) {
            throw Foswiki::OopsException(
                'attention',
                def    => 'no_form_def',
                web    => $session->{webName},
...
sub getAvailableForms {
	...
	my @forms = @{$metaObject->{_session}->{store}->getAvailableForms()};
...
=pod	
	...
	my @WTforms;
	foreach my $formKey (@forms) {
		my ($web,$topic) = Foswiki::Func::getWTFromTopicKey($formKey);
		push(@WTforms,"$web.$topic");
	}
=cut
	return @forms;
}
    ...
    foreach my $line ( split( /\n/, $text ) ) {
       ...
       # Only insist on first field being present FIXME - use oops page instead?
        if ( $inBlock && $line =~ s/^\s*\|\s*// ) {
            ...
            $vals ||= '';
            #$vals = $this->expandMacros($vals);# <-- this is bad, b/c we can't put searches in the Value section
...
sub renderForEdit {
    ...
    foreach my $fieldDef ( @{ $this->{fields} } ) {
		...
		# temporary?????? TODO???
		$fieldDef->{value} = $topicObject->expandMacros($fieldDef->{value}) if defined $fieldDef->{value};
